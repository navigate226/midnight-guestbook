...
import { Guestbook, type GuestbookPrivateState, witnesses } from '@midnight-ntwrk/guestbook-contract';
...
import {
  type GuestbookContract,
  type GuestbookPrivateStateId,
  type GuestbookProviders,
  type DeployedGuestbookContract,
} from './common-types';
...
export const getGuestbookCounter = async (
    providers: GuestbookProviders,
    contractAddress: ContractAddress
): Promise<bigint | null> => {
    assertIsContractAddress(contractAddress);
    logger.info("Checking contract address ledger state");

    const guestbookCounter = await providers.publicDataProvider
      .queryContractState(contractAddress)
      .then((contractState) => (contractState != null ? Guestbook.ledger(contractState.data).guestbookCounter : null));

    logger.info(`Guestbooks amount: ${guestbookCounter}`)
    
    return guestbookCounter;
};
...
export const getMessageCounter = async (
    ...
    const messageCounter = await providers.publicDataProvider
      .queryContractState(contractAddress)
      .then((contractState) => (contractState != null ? Guestbook.ledger(contractState.data).messageCounter : null));

    logger.info(`Messages amount: ${messageCounter}`)
...
export const guestbookContractInstance: GuestbookContract = new Guestbook.Contract(witnesses)

export const deploy = async (
  providers: GuestbookProviders,
  privateState: GuestbookPrivateState
): Promise<DeployedGuestbookContract> => {
  logger.info("Deploying guestbook contract...");

  const guestbookContract = await deployContract(providers, {
    contract: guestbookContractInstance,
    privateStateId: 'guestbookPrivateState',
    initialPrivateState: privateState
  });

  logger.info(`Deployed contract at address: ${guestbookContract.deployTxData.public.contractAddress}`);
  return guestbookContract;
};

// join contract
// not clear what should the private state be initialized to in my contract

// functions that execute circuits, like create guestbook, write guestbook, read guestbook or archiveguestbook
export const displayGuestbookCounter = async (
  providers: GuestbookProviders,
  guestbookContract: DeployedGuestbookContract
): Promise<{ guestbookCounter: bigint | null; contractAddress: string }> => {
  const contractAddress = guestbookContract.deployTxData.public.contractAddress;
  const guestbookCounter = await getGuestbookCounter(providers, contractAddress);

  if (guestbookCounter == null) {
    logger.info(`There are no guestbooks deployed at ${contractAddress}`);
  } else {
    logger.info(`Deployed guestbooks at ${contractAddress}: ${Number(guestbookCounter)}`);
  }

  return { guestbookCounter, contractAddress };
};

export const displayMessageCounter = async (
  providers: GuestbookProviders,
  guestbookContract: DeployedGuestbookContract
): Promise<{ messageCounter: bigint | null; contractAddress: string }> => {
  const contractAddress = guestbookContract.deployTxData.public.contractAddress;
  const messageCounter = await getMessageCounter(providers, contractAddress);

  if (messageCounter == null) {
    logger.info(`There are no messages written at ${contractAddress}`);
  } else {
    logger.info(`Written messages at ${contractAddress}: ${Number(messageCounter)}`);
  }

  return { messageCounter, contractAddress };
};

// function to create a wallet in CLI 
...
export const configureProviders = async (wallet: Wallet & Resource, config: Config) => {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider<typeof GuestbookPrivateStateId>({
