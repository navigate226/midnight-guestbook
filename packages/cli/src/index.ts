...
const resolve = async (
  ...
  rli: Interface,
  logger: Logger
): Promise<GuestbookAPI | null> => {
  let api: GuestbookAPI | null = null;

  while (true) {
    const choice = await rli.question(DEPLOY_OR_JOIN_QUESTION);
    switch (choice) {
      case "1":
        api = await GuestbookAPI.deployGuestbookContract(
          providers,
          logger
        );
        logger.info(
      ...
      case "2":
        api = await GuestbookAPI.joinGuestbookContract(
          providers,
...
const displayLedgerState = async (
  providers: GuestbookContractProviders,
  deployedStateraContract: DeployedGuestbookOnchainContract,
  logger: Logger
): Promise<void> => {
...
const getUserPrivateState = async (
  providers: GuestbookContractProviders
): Promise<GuestbookPrivateState | null> =>
  providers.privateStateProvider
    .get(GuestbookPrivateStateId)
...
const displayUserPrivateState = async (
  ...
  if (privateState === null)
    logger.info(
      `There is no private state stored at ${GuestbookPrivateStateId}`
    );
...
async function askNumber(prompt: string, rli: Interface): Promise<number> {
  ...
  if (isNaN(num)) throw new Error(`Invalid number: ${answer}`);
  return num;
}
...
const circuit_main_loop = async (
  ...
  const GuestbookAPI = await resolve(providers, rli, logger);
  if (GuestbookAPI === null) return;

  let currentState: DerivedGuestbookContractState | undefined;
  const stateObserver = {
    next: (state: DerivedGuestbookContractState) => {
      currentState = state;
    },
  };
    ...
    while (true) {
      ...
      switch (choice) {
        ...
        case "5": {
          ...
          logger.info("Waiting for wallet to sync after creating guestbook...");
...
const displayComprehensiveWalletState = async (
  wallet: Wallet,
  currentContractState: DerivedGuestbookContractState | undefined,
  logger: Logger
): Promise<void> => {
  const state = await Rx.firstValueFrom(wallet.state());
...
export const run = async (
  ...
  if (dockerEnv !== undefined) {
    ...
    if (config instanceof StandaloneConfig) {
      ...
      config.node = mapContainerPort(env, config.node, "guestbook-node");
      config.proofServer = mapContainerPort(
        env,
        config.proofServer,
        "guestbook-proof-server"
      );
    }
  }
  ...
  try {
    if (wallet !== null) {
      ...
      const providers: GuestbookContractProviders = {
        ...
        zkConfigProvider: new NodeZkConfigProvider<TokenCircuitKeys>(
          config.zkConfigPath
        ),
        
        proofProvider:
          httpClientProofProvider<TokenCircuitKeys>(config.proofServer) as unknown as GuestbookContractProviders["proofProvider"],
